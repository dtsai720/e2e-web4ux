...
test.describe("Validate Project", () => {
    ...
    test("Happy Path", async ({ page }) => {
        ...
        await page
            .locator(
                '#task-orders > div[data-task="winfitts"] > div.title > span.icon-toggle.collapse'
            )
            .click();
        await page
            .locator(
                '#task-orders > div[data-task="typing"] > div.title > span.icon-toggle.collapse'
            )
            .click();
        await page.waitForSelector('#task-orders > div[data-task="typing"]');

        const typingSelector = await page.locator(
            '#task-orders > div[data-task="typing"] > div.body'
        );
        ...
        await page.waitForSelector(
            "div.main-content > div.modal.documentup-modal.show > div > div.modal-body"
        );
        await page
            .locator(
                "div.main-content > div.modal.documentup-modal.show > div > div.modal-body > div.selects-1 > div > div > div"
            )
            .click();
        ...
        const actualModelName = await page
            .locator(".step4Device > p > span:nth-child(1)")
            .textContent();
        const actualDeviceName = await page
            .locator(".step4Device > p > span:nth-child(2)")
            .textContent();
        const step4Task = await page.locator(".step4Tasks");
        const actualTypingTotalTimer = await step4Task
            .locator("div:nth-child(3) > p:nth-child(2) > span")
            .textContent();
        const actualCountdownTimer = await step4Task
            .locator("div:nth-child(3) > p:nth-child(3) > span")
            .textContent();
        const actualSelectDocument = await step4Task
            .locator("div:nth-child(3) > p:nth-child(5) > span")
            .textContent();
